...
| --------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------- |
| VLA                                     | Опциональная (до C23) поддержка массивов переменной длины (Variable Length Array, C99 п. 6.7.5.2), может быть аргументом функции, размещаться в куче или иметь автоматическое размещение (в блоке).<br><br>Лично мне известен только один компилятор, который определяет макрос `__STDC_NO_VLA__`                                         | `void fvla(int m, int n, int (*a)[m][n])` |
...
| ZLA                                     | Расширение языков C/C++ в части массивов нулевой длины (Zero Length Array). Весьма распространённое расширение, два из трёх компиляторов его поддерживают                                                                                                                                                                                 | `int z0[] = {};`                          |
|                                         |                                                                                                                                                                                                                                                                                                                                           | `int z02[0][2];`                          |
...
Лично мне, кажется, что последней соломинкой, которая  переломила консерватизм комитета WG14, был вопрос производительности. При работе с VLA, у которых младшие размерности переменны, компиляторы не сокращают общие подвыражения в классической идиоме, а много и часто делят (следствие пп. 2 и 4, ниже). Следует заметить, что хотя примеры VLA проще давать в виде создания их на стеке, но VLA ,как аргументы функций и/или как результат выделения в куче, не менее важны.
...
Все известные компиляторы, которые поддерживают VLA, расширяют C, в части возможности создания и/или ссылки на VLA размера 0, без каких-либо предупреждений или ограничений:
<!-- example: "vla0.c" -->
```c
...
Правда, при использовании этих двух вариантов, элементы нулевого размера всё равно придётся как-то из диеты исключить, вероятно, дисциплинарными методами. И следить за этим вопросом при рефакторинге.
...
Формально, использование оператора рефлексии `^^decltype(a)` позволяет получить свойства типа минуя сопоставление с шаблоном, что даёт возможность избавится от вызовов встроенных функций. Но, для получения числа элементов фиксированных массивов, ZLA или контейнеров,  существенных улучшений не видно:
https://godbolt.org/z/oPWbhE49W
<!-- example: "c++26.cpp" -->
```c++
constexpr size_t unthinkable = 1917;
constexpr size_t bias = 1;
...
Таким образом, в C++26, так же как в C++14, невозможно получить число элементов VLA с элементами нулевого размера, в этом случае можно только вернуть 0. Но, есть шанс, что в C++26 это можно будет сделать более менее "чисто". Спецификации интерфейсов этого не запрещают, но не и обязывают, поскольку не специфицируют реализацию std::meta::info.
