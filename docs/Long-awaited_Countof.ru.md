...
В отличие от классической идиомы, используется проверка на нулевой размер элемента. Для тех компиляторов, которые не поддерживают VLA и не поддерживают расширений ZLA (например, для MSVC), эта проверка не требуется. но и вреда от неё никакого.
