...
Макрос `countof_ns(array)` возвращает количество элементов в своем операнде (массив или тип массива). Количество элементов определяется типом операнда. Результатом является целое число. Если количество элементов является переменным, операнд вычисляется, в противном случае операнд не вычисляется, и результат является целым константным выражением.
