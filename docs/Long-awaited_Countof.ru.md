...
Однако, большинство современных компиляторов, выражение `_countof(int [7])` раскроют в  `(sizeof(int [7]) / sizeof(int [7][0]))` и вставят в генерируемый код - деление на 0. Возможно, сопроводят это дело предупреждением (поделят на ноль, или иное UB учинят, почти все, а вот предупреждение выдадут далеко не все). Поэтому в новом коде чаще встречаются варианты `sizeof(*(a))` или `sizeof((a)[0])`.
