...
| EMPTY_STRUCTURE<br>ALONE_FLEXIBLE_ARRAY | Пустые структуры (расширение C), структуры содержащие единственный завершающий массив неполного типа (flexible array), объединения нулевого размера (расширение C/C++).                                                                                                                                                                                                                                | `struct {} s0;`<br><br>`struct { int flex[]; } af0;`<br><br>`union { int i0[0]; char c0[0]; } u0;`            |
## Жизнь до `_Countof`
