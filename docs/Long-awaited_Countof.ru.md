...
В отличие от классической идиомы, используется проверка на нулевой размер элемента, грубо говоря, если деление на 0 не определено, то пусть будет 0 в этом случае. Для тех компиляторов, которые не поддерживают VLA и не поддерживают расширений ZLA (например, для MSVC), эта проверка не требуется, но и вреда от неё никакого.
...
К сожалению, оба этих варианта теряют предупреждения компиляторов о делении на 0, в случае нулевых размеров элементов фиксированных массивов. Если таковые предупреждения представляются достаточно ценными, можно в случае `__builtin_constant_p(sizeof((a)[0]))` проверить `sizeof((a)[0])`, без нарушения константности выражения, и породить собственное предупреждение.
