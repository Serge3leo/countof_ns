...
### ZLA и шаблоны
К сожалению, все компиляторы единодушны, `std::size()` не приемлет ZLA расширений C++, хотя для контейнеров, он успешно возвращает 0. Так же, у всех, свойство `std::extent_v<>`, для ZLA возвращает, собственно, оно возвращает 0 для любых типов, которые не являются `std::is_bounded_array_v<>`, даже для `void`.

А вот в вопросе свойства `std::is_array_v<>`, и сопоставления с шаблонами, мнения компиляторов расходятся:
<!-- example: "zla_cxx.cpp" -->
```c++
int zla[0];
...
В общем, для ZLA нет общепринятых шаблонов свойств, необходимо своё. Кроме того, по аналогии с `std::size()`, будем считать контейнером любой тип, который имеет метод `size()`. Для того, что бы аргумент не вычислялся, используем `sizeof()` от разыменованного возвращаемого значения, за исключением случая контейнера. Для него вызовем `size()` и, при его вызове, аргумент будет вычислен:
<!-- example: "countof_ns.h" -->
```c++
...
```
<!-- endexample: "countof_ns.h" -->
Результаты `_countof_ns(a)` идентичны результатам оператора `_Countof` проекта C2Y для аналогичных массивов.
### VLA и `__is_same()`
