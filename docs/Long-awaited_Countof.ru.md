...
Предупреждение Clang/IntelLLVM выглядит достаточно капризным, по крайней мере, мне не удалось его применить для диагностики того, что аргумент идиомы имеет побочный эффект (`((void)sizeof((a), sizeof(a)), sizeof(a) / sizeof(a[0]))`).
...
```
<!-- endexample: "zla.c" -->
В части идиомы количества элементов массива, компилятор, в лучшем случае, выдаст предупреждение о делении на 0 (в ряде случаев, он даже предупреждения не выдаст), но код скомпилируется и вызовет SIGFPE/SIGSEGV или иное проявление UB во время выполнения. Т.е. надо за этим как-то следить, что размер элемента 0 невозможен.
...
## Реализация `countof()`
В предложении [N3369](https://www.open-std.org/JTC1/SC22/WG14/www/docs/n3369.pdf) описано почему  точная реализация макроса `countof()` без оператора `_Countof` на практике - невозможна. Рассмотрим эти причины на предмет возможности реализации более менее близкого переносимого аналога без UB.
...
Сама эта функция достаточно распространена и, если использовать её использовать в позитивном варианте:
```c
#if __STDC_VERSION__ >= 202311L
...
```c
#define countof_ns(a)  _countof_ns(*(_countof_ns_typeof(a) *)8192)
```
...
Второй момент, по определению, в этом случае `_Countof` не должен вычислять свой аргумент, а любые полезные функции:  `typeof(a)`, `sizeof(a)`, `auto v = (a)`, `typedef typeof(a) t_t` и т.п., его должны вычислить.
...
- Некоторые компиляторы не поддерживают в операторах-выражениях использование VLA или указатели на них.
