...
Как заметил один мой мудрый товарищ, Алексей Годин, даже полвека ещё не прошло, а наконец, дождались. В проекте стандарта языка C2y появился оператор числа элементов.
## Оператор `_Countof`
Формально, согласно п. 6.5.4.5 [N3685](https://www.open-std.org/jtc1/sc22/wg14/www/docs/n3685.pdf), представим виновника торжества.
...
## Жизнь до `_Countof`
В ранних языках программирования с числом элементов массива традиционно имелись некоторые сложности. Считалось, что если программист определил массив `A[N]`, то он без труда может просто использовать `N`.
...
Однако, большинство современных компиляторов, выражение `_countof(int [7])` раскроют в  `(sizeof(int [7]) / sizeof(int [7][0]))` и вставят в генерируемый код - деление на 0. Возможно, сопроводят это дело предупреждением (поделят на ноль, или иное UB учинят, почти все, а предупреждение выдадут далеко не все). Поэтому в новом коде чаще встречается варианты `sizeof(*(a))` или `sizeof((a)[0])`.
...
Но это неточно. Во-первых, все компиляторы, которые поддерживают стандартные VLA языка C, реализуют VLA расширение языка C++, но в части побочных эффектов оператора `sizeof`  в расширениях C++ согласия нет. Во-вторых, не все компиляторы точно реализуют стандарты C99...C23.
...
```
<!-- endexample: "div0.c" -->
Соберётся без единого предупреждения, но, конечно, 1917 не напечатает (увы, вечно эти Clang и Intel всё оптимизируют исходя из отсутствия UB, когда надо общее подвыражение не могут вынести и сократить 😒). Таким образом, надо за этим как-то отдельно следить, что размер элемента 0 невозможен.
...
#endif
```
Во-первых, вместо многоточия можно использовать обычную классическую идиому. Сообщество BSD как раз так и живёт, при использовании Сlang (`clang`) (или IntelLLVM (`icx`) или GNU (`gcc -Wall`)) будут выдаваться предупреждения или даже ошибки для большинства случаев некорректного использования.
...
#elif !_COUNTOF_NS_BROKEN_TYPEOF
    #define _countof_ns_typeof(t)  __typeof__(t)
#else
    #define _countof_ns_typeof(t)  const volatile __typeof__(t)
#endif
	// Обход неопределённости деления 0/0
#define _countof_ns_unsafe(a)  (0 == sizeof(*(a)) ? 0 : sizeof(a)/sizeof(*(a)))
...
- Практическое отсутствие UB. Строго говоря, согласно C11...С23 неопределённым поведением является, как несовпадение спецификаторов размера массивов используемых в операции, так и его равенство 0. Однако, такое может случится только в случае когда `a` является расширением языка C на массивы с нулевым размером элементов, и на практике, у таковых расширений, поведение выглядит вполне определённым;
Минусы:
...
По определению, `_Countof` от фиксированного массива `a` должен быть целочисленной константой, даже если он содержит VLA. Но `sizeof(a)` и `sizeof(*a)` не являются константами, и современные компиляторы не считают `sizeof(a)/sizeof(*a)` константой.

Второй момент, по определению, в этом случае `_Countof` не должен вычислять свой аргумент, а любые полезные конструкции:  `typeof(a)`, `sizeof(a)`, `auto v = (a)`, `typedef typeof(a) t_t` и т.п., его должны вычислить.
...
По всей видимости, нет способа реализации макроса `countof_ns()` для случая аргумента с побочными эффектами.

**TODO**: Интересно, а есть ли способ, для выражения языка C, запретить или диагностировать наличие побочных эффектов?
- Предупреждения Clang/IntelLLVM: unevaluated-expression, вроде как, в подстановке макросов не работают, работают только свободно;
- Локальные функции GNU с атрибутами `[[reproducible, unsequenced]]` тоже не порождают предупреждений;
- Похоже остаётся только специализированный анализатор забабахать 😉 Clang-Tidy & PPCallbacks ?
- Clang-Tidy и bugprone-assert-side-effect в помощь?
## countof_ns()
Уж что вышло, то вышло
## Реализация `countof()` в  C++
