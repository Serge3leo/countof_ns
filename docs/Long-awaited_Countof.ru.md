...
Как можно заметить, что, и в K&R, и у MS Visual Studio `_countof()`, имя массива не бралось в скобки. Для компиляторов, строго следующих стандарту языков C/C++, в этом нет необходимости, т.к. массивы размерности 0 недопустимы.
...
К сожалению, оба этих варианта теряют предупреждения компиляторов о делении на 0, в случае известных нулевых размеров элементов. Если таковые предупреждения представляются достаточно ценными можно, либо сначала проверить `__builtin_constant_p(sizeof((a)[0]))`, а потом проверить `sizeof((a)[0])`, уже без нарушения константности выражения, и породить собственное предупреждение или ошибку, либо воспользоваться другими реализациями, описанными ниже, у которых некорректное разрешение неопределённости 0 на 0 приводит к ошибке компиляции.
...
У GNU (`gcc`) есть плохо документированная особенность: предупреждения `-Wsizeof-pointer-div` и `-Wsizeof-array-argument` подавляются для файлов заголовков, которые найдены по системным путям `/usr/include`, `/usr/local/include` (пути зависят от системы, я не нашёл точного описания этой особенности, буду благодарен за любые сведения о ней).
...
- Это только частный случай, поскольку не обеспечивает полного исключения побочных эффектов `typeof(a)` для предыдущего случая фиксированного массива элементов VLA;
- GNU расширение оператора-выражения неприменимо вне тела функции;
